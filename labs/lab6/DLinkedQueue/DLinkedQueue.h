...
template <typename T>
void DLinkedQueue<T>::push(const T& item)
{
	//if empty
	if (qsize == 0)
	{
...
template <typename T>
const T& DLinkedQueue<T>::front() const
{
	if (qsize == 0)
	throw underflowError("Empty");
	return headPtr->nodeValue;
...
template <typename T>
void DLinkedQueue<T>::printQueue()
{
	node<T>* temp = headPtr;
...
template <typename T>
void DLinkedQueue<T>::printBackwards()
{
	node<T>* temp = tailPtr;
	for (int i = 0; i < qsize; i++)
	{
...
template <typename T>
int DLinkedQueue<T>::size()
{
	return qsize;
}

template <typename T>
bool DLinkedQueue<T>::empty()
{
	if (qsize == 0)
		return true;
	return false;
