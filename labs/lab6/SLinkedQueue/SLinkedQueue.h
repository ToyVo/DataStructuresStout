...
class SLinkedQueue
{
    public:
		...
		//Postcondition: Queue size decremented and front element removed and memory for node returned to the heap
		void pop();
...
template <typename T>
void SLinkedQueue<T>::push(const T& item)
{
	//if empty
	if (qsize == 0)
	{
...
template <typename T>
T& SLinkedQueue<T>::front()
{
	return headPtr->nodeValue;
}

template <typename T>
const T& SLinkedQueue<T>::front() const
{
	return headPtr->nodeValue;
}

template <typename T>
void SLinkedQueue<T>::pop()
{
	//if empty
	if (qsize == 0)
	{
...
template <typename T>
void SLinkedQueue<T>::printQueue()
{
	node<T>* temp = headPtr;
...
template <typename T>
int SLinkedQueue<T>::size()
{
	return qsize;
}

template <typename T>
bool SLinkedQueue<T>::empty()
{
	if (qsize == 0)
		return true;
	return false;
