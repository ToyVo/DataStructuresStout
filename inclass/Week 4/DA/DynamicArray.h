...
class DynamicArray{
	public:
		...
		// copy constructor uses existing DynamicArray obj to create new DynamicArray(*this)
		// Postcondition: creates current DynamicArray (*this) as a copy of obj
		DynamicArray(const DynamicArray& obj);

		// destructor
		// Postcondition: the dynamic array is destroyed and memory returned to the heap
		~DynamicArray();
		...
		// Postcondition: value at index n with the rest of the elements moved up and size+1
...
template <class T>
void DynamicArray::growArray(int n, bool copy)
{z
...
DynamicArray::DynamicArray(int capacity)
{
	...
	//use growArray to set capacity and initialize array, but no elements to copy.
...
template <class T>
bool operator== ( DynamicArray& lhs,  DynamicArray& rhs)
{
...
template <class T>
ostream& operator<<(ostream& ostr, const DynamicArray& da)
{
...
template <class T>
void DynamicArray::set(int value, int index)
{
	vArr[index] = value;
}
...
template <class T>
void DynamicArray::printVector() const
{
